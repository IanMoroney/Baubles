...
public class CommandBaubles {
    ...
    private static int clear(CommandSource source, ServerPlayerEntity target, int slot) {
        target.getCapability(BaublesCapabilities.CAPABILITY_BAUBLES).ifPresent(b -> {
            ...
            for (int i : toClear) {
                ...
                source.sendFeedback(new TranslationTextComponent("command.baubles.clear.feedback", i, target.getDisplayName()), true);
                target.sendMessage(new TranslationTextComponent("command.baubles.clear.cleared", i, source.getDisplayName()), null);
            }
        });
        return Command.SINGLE_SUCCESS;
    }

    private static int show(CommandSource source, ServerPlayerEntity target, int slot) {
        target.getCapability(BaublesCapabilities.CAPABILITY_BAUBLES).ifPresent(b -> {
            int[] toShow = { slot };
            if (slot == -1) {
            ...
            for (int i : toShow) {
                source.sendFeedback(new TranslationTextComponent("command.baubles.show.result", target.getDisplayName(), i, b.getStackInSlot(i).getTextComponent()), true);
            }
        });
        return Command.SINGLE_SUCCESS;
    }
